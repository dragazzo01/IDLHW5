...
class DDPMScheduler(nn.Module):
    ...
    def __init__(
        ...
        if self.beta_schedule == 'linear':
            ...
            # betas = [beta_start + ((beta_end - beta_start)*(t-1))/(num_train_timesteps) for t in num_train_timesteps]
    ...
    def set_timesteps(
        ...
        # timesteps = [ratio*i for i in range(num_inference_steps)]
        # timesteps.reverse()
        # timesteps = np.array(timesteps)
        
        self.num_inference_steps = num_inference_steps
        self.timesteps =  torch.tensor([ratio*i for i in range(num_inference_steps)], dtype=torch.int32).to(device)
        # self.timesteps = torch.linspace(1, self.num_train_timesteps, self.num_inference_steps, dtype=torch.int)
        # self.timesteps = torch.flip(self.timesteps, dim=0)
    ...
    def previous_timestep(self, timestep):
        ...
        # timesteps = self.timesteps.cpu().detach().numpy()
        # # timestep=
        # print(timesteps)
        # print(timestep)
        indecies = (self.timesteps == timestep).nonzero(as_tuple=True)[0]

        if len(indecies) == 0 or indecies[0] == 0:
            print("uhohh")
            return self.timesteps[0]
        
        prev_t = self.timesteps[indecies[0]-1]
        return prev_t
    ...
    def add_noise(
        ...
        sqrt_one_minus_alpha_prod = 1.0 - alphas_cumprod[timesteps]
        sqrt_one_minus_alpha_prod = torch.sqrt(sqrt_one_minus_alpha_prod)
    ...
    def step(
        ...
        # prev_t = self.previous_timestep(t).to(torch.int)
        prevt_t = t-1
        
        # TODO: 1. compute alphas, betas
        # alpha_prod_t = self.alphas_cumprod[t]
        # alpha_prod_t_prev = self.alphas_cumprod[prev_t] 
        # betas_prod = torch.cumprod(self.betas, dim=0)
        # beta_prod_t = betas_prod[t]
        # beta_prod_t_prev = betas_prod[prev_t] 
        ...
        # pred_original_sample_coeff = (torch.sqrt(alpha_prod_t_prev)*current_beta_t)/(1.0-alpha_prod_t_prev)
