...
class RescaleToMinusOneOne:
    def __call__(self, img):
        return img * 2 - 1  # Assuming img is already normalized to [0, 1]   
...
def main():
    ...
    # send to device
    unet = unet.to(device)
    ddpm_scheduler = ddpm_scheduler.to(device)
    if vae:
        vae = vae.to(device)
    if class_embedder:
    ...
    pipeline = DDPMPipeline(unet, ddpm_scheduler)
    ...
    # start tracker
    if is_primary(args) and args.wandb:
    ...
    for epoch in range(args.num_epochs):
        ...
        if args.use_cfg:
            ...
            # TODO: fill pipeline
            gen_images = pipeline(batch_size=args.batch_size, num_inference_steps=args.num_inference_steps) 
